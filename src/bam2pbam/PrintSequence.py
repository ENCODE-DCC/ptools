...
class Lookup: 
        def __init__(self, f):
                ...
                for record in SeqIO.parse(f, "fasta"): #From http://biopython.org/wiki/SeqIO 
                        self.chroms[record.id]=record.seq #Filling the dictionary with chromosomes as keys, and the sequence as the value.
#                        print("read %s" % record.id) #Lets us know when each piece of the object is done loading
 ...
 #This query function will automatically fill in the 'self' argument, but will then accept the chromosome number as 'chr$c'--where $c is a natural number, pos is the position number, and len is the number of basepairs
        ...
        def query(self, chrom, pos, len):
                return str(self.chroms[chrom][pos:pos+len])# str returns a string version of our output. Our output is the object we created, the dictionary using key chrom, and indexed from our starting position to our position plus number of basepairs.

#This numbps function will accept a chromosome number, as 'chr1' for example. 
        def numbps(self, chrom):
                return len(self.chroms[chrom]) #This returns the length (count) of the basepairs of the value assigned to the chroms dictionary under the chrom key.
